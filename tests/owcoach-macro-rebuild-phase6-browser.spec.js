...
async function expectPhaseBlocks(page) {
  ...
  await expect(result).toContainText('シナジー');
  await expect(result).toContainText('敵構成の勝ち方');
  await expect(result).toContainText('先に止める連携');
  await expect(result).toContainText('主な勝ち方 / 次に警戒する形');
...
test('phase 5 synthesis is visible in all result modes', async ({ page }) => {
  for (const mode of ['quick', 'standard', 'detail']) {
    ...
    await expect(page.locator('#result')).toContainText('ダイブ連携');
  }
});
...
test('phase 4 detail surface stays visible after phase 5 synthesis', async ({ page }) => {
  ...
  await expect(detail).toContainText('この敵の役割');
  await expect(detail).toContainText('安全な立ち位置');
  await expect(detail).toContainText('先に止める相手');
  await expect(detail).toContainText('シナジー確認');
